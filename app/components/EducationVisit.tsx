...
const copy = {
  pt: {
    ...
    demo: {
      ...
      correct: "Você reconheceu a colocação natural: ask a question. O verbo já carrega a ação de formular a pergunta.",
      doFeedback: "“Do” herda a lógica de “fazer uma pergunta”. O sistema identifica transferência do português, não falta de vocabulário.",
      makeFeedback: "“Make” costuma criar ou produzir algo. Aqui, a ação linguística convencional é ask a question.",
    ...
    model: {
      ...
      stages: [
        ["Observação", "Registrar a expressão escolhida sem apagar a intenção comunicativa."],
        ["Diagnóstico", "Separar transferência lexical, colocação e domínio da estrutura interrogativa."],
        ["Feedback", "Explicar ask a question dentro da frase, sem reduzir a devolutiva a certo ou errado."],
        ["Revisão", "Testar a mesma construção com novo sujeito, contexto e distância temporal."],
      ],
    ...
    synthesis: {
      ...
      principles: [["01", "Erro como evidência"], ["02", "Feedback como mecanismo"], ["03", "Revisão como transferência"]],
  ...
  en: {
    ...
    demo: {
      ...
      instruction: "Complete the sentence. The feedback changes with your hypothesis.",
      sentenceStart: "Can I",
      sentenceEnd: "a question before we start?",
      prompt: "Choose an expression",
      correct: "You recognized the natural collocation: ask a question. The verb already carries the act of formulating the question.",
      doFeedback: "“Do” inherits the logic of the Portuguese phrase. The system identifies language transfer, not missing vocabulary.",
      makeFeedback: "“Make” usually creates or produces something. Here, the conventional language action is ask a question.",
    ...
    model: {
      ...
      stages: [
        ["Observation", "Record the chosen expression without erasing the communicative intention."],
        ["Diagnosis", "Separate lexical transfer, collocation and command of the question structure."],
        ["Feedback", "Explain ask a question inside the sentence instead of reducing feedback to right or wrong."],
        ["Review", "Test the same construction with a new subject, context and time interval."],
      ],
    ...
    synthesis: {
      ...
      principles: [["01", "Error as evidence"], ["02", "Feedback as mechanism"], ["03", "Review as transfer"]],
...
export default function EducationVisit({ language }: { language: Language }) {
  ...
  const close = useCallback(() => {
    setScene(null);
    window.history.pushState(null, "", `${window.location.pathname}${window.location.search}#education-visit`);
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === "ArrowRight" && scene < sceneCount - 1 && (scene !== 1 || answer)) {
        event.preventDefault();
        openScene(scene + 1);
        return;
      }
      if (event.key !== "Tab" || !stageRef.current) return;
      const focusable = Array.from(stageRef.current.querySelectorAll<HTMLElement>('button:not([disabled]), a[href], [tabindex]:not([tabindex="-1"])'));
  ...
  const feedback = answer === "ask" ? t.demo.correct : answer === "make" ? t.demo.makeFeedback : t.demo.doFeedback;
  const observedAnswer = answer ?? "do";

  return (
    <>
      <section className="education-visit-entry shell" id="education-visit" aria-labelledby="education-visit-title">
        <p className="eyebrow">{t.entry.eyebrow}</p>
        <div>
          <h2 id="education-visit-title">{t.entry.title}</h2>
          <p>{t.entry.text}</p>
        </div>
        <button ref={launchRef} type="button" onClick={() => openScene(0)}>{t.entry.action}<span aria-hidden="true">→</span></button>
      </section>

      {scene !== null ? (
        <div ref={stageRef} className="education-stage" role="dialog" aria-modal="true" aria-labelledby="education-stage-title">
          <header className="education-stage-header">
            <VictorMonogram className="education-stage-mark" />
            <p>{t.chrome.label}</p>
            <button type="button" onClick={close}>{t.chrome.close}<span aria-hidden="true">×</span></button>
          </header>

          <div className="education-stage-progress" aria-label={`${t.chrome.scene} ${scene + 1} / ${sceneCount}`}>
            <span>{String(scene + 1).padStart(2, "0")}</span>
            <div>{Array.from({ length: sceneCount }, (_, index) => <i className={index <= scene ? "is-reached" : ""} key={index} />)}</div>
            <span>{String(sceneCount).padStart(2, "0")}</span>
          </div>

          <main className="education-stage-body" key={scene}>
            {scene === 0 ? (
              <section className="education-scene education-scene-problem">
                <div className="education-scene-copy">
                  ...
                  <h2 ref={headingRef} tabIndex={-1} id="education-stage-title">{t.problem.title}</h2>
                  <p>{t.problem.text}</p>
                </div>
                <div className="education-problem-proof" aria-label={t.problem.label}>
                  <span>{t.problem.label}</span>
                  <p>Can I <strong>do</strong> a question before we start?</p>
                  <i>{t.problem.note}</i>
                </div>
              </section>
            ) : null}
            ...
            {scene === 1 ? (
              <section className="education-scene education-scene-demo">
                <div className="education-scene-copy">
                  ...
                  <h2 ref={headingRef} tabIndex={-1} id="education-stage-title">{t.demo.title}</h2>
                ...
                <div className="education-demo" aria-label={t.demo.prompt}>
                  <div className="education-demo-sentence">
                    ...
                    <strong className={answer ? "has-answer" : ""}>{answer ?? "_____"}</strong>
                    <span>{t.demo.sentenceEnd}</span>
                  </div>
                  <div className="education-demo-options" role="group" aria-label={t.demo.prompt}>
                    {(["do", "make", "ask"] as Answer[]).map((choice) => (
                      <button type="button" aria-pressed={answer === choice} className={answer === choice ? "is-selected" : ""} onClick={() => setAnswer(choice)} key={choice}>{choice}</button>
                    ))}
                  </div>
                  {answer ? (
                    <div className={`education-demo-feedback ${answer === "ask" ? "is-correct" : ""}`} aria-live="polite">
                      <span>{t.demo.mechanism}</span>
                      <p>{feedback}</p>
                      <button type="button" onClick={() => setAnswer(null)}>{t.demo.reset}</button>
                    </div>
            ...
            {scene === 2 ? (
              <section className="education-scene education-scene-model">
                <header>
                  ...
                  <h2 ref={headingRef} tabIndex={-1} id="education-stage-title">{t.model.title}</h2>
                  <p>{t.model.text}</p>
                </header>
                <div className="education-decision-line">
                  {t.model.stages.map(([title, text], index) => (
                    <article key={title}><span>0{index + 1}</span><h3>{title}</h3><p>{index === 0 ? `${text} “${observedAnswer} a question”.` : text}</p></article>
                  ))}
                </div>
                <div className="education-return"><span>{t.model.returnLabel}</span><p>{t.model.returnSentence}</p></div>
              </section>
            ) : null}
            ...
            {scene === 3 ? (
              <section className="education-scene education-scene-synthesis">
                <div className="education-scene-copy">
                  ...
                  <h2 ref={headingRef} tabIndex={-1} id="education-stage-title">{t.synthesis.title}</h2>
                  <p>{t.synthesis.text}</p>
                </div>
                <ol>{t.synthesis.principles.map(([number, label]) => <li key={number}><span>{number}</span>{label}</li>)}</ol>
                <div className="education-synthesis-actions">
                  <a href={contactHref} target="_blank" rel="noreferrer">{t.synthesis.cta}<span aria-hidden="true">↗</span></a>
                </div>
              </section>
            ) : null}
          </main>
          ...
          <footer className="education-stage-footer">
            <button type="button" onClick={() => openScene(scene - 1)} disabled={scene === 0}><span aria-hidden="true">←</span>{t.chrome.previous}</button>
            <span>{t.chrome.scene} {scene + 1} / {sceneCount}</span>
            {scene < sceneCount - 1 ? (
              <button type="button" className="education-stage-next" onClick={() => openScene(scene + 1)} disabled={scene === 1 && !answer}>{t.chrome.next}<span aria-hidden="true">→</span></button>
            ) : <button type="button" className="education-stage-next" onClick={close}>{t.chrome.close}<span aria-hidden="true">×</span></button>}
          </footer>
        </div>
      ) : null}
    </>
